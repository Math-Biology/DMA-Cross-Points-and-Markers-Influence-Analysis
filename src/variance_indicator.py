...
For each (visit, point) series with detected positives, computes the sample
variance (ddof=1) of two segments per anchor (REQ-CPM-IA-P26.0009):
...
Returns an empty list for series with no_cross_marker_effect=True.
...
class VarianceIndicator:
    """Immutable pre/post-anchor variance indicator for one anchor."""
...
VarianceIndicators = Dict[Tuple[str, str], List[Optional[VarianceIndicator]]]
...
def compute_variance_indicators(
    ...
    """
    Compute pre/post-anchor sample variance indicators for every anchor.
    ...
    Returns:
        VarianceIndicators: mapping (visit_id, point_id) -> List[Optional[VarianceIndicator]].
    ...
    for key, result in trigger_results.items():
        ...
        vi_list: List[Optional[VarianceIndicator]] = []
        for i, anchor in enumerate(result.anchors):
            before_clean = [
                v for v in sequences[key][0:anchor.index]
                if not math.isnan(v)
            ]
            after_clean = [
                v for v in descent_windows[key][i].window_values
                if not math.isnan(v)
            ]
            ...
            vi_list.append(VarianceIndicator(
                variance_before=variance_before,
                variance_after=variance_after,
                variance_ratio=variance_ratio,
            ))
        indicators[key] = vi_list

    logger.debug(
        "CPM-IA variance indicators | total=%d | with_indicators=%d",
        len(indicators),
        sum(1 for v in indicators.values() if v),
    )
