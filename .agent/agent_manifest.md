...
**Short Code:** CPM-IA
**SR&TS Document:** `#L001-U015-P26.0127 Rev 00.01` — 23 September 2026
...
2. Detects **all positive anchors** — all markers whose raw percentage variation is >= a configurable threshold (default 300 %).
...
| G-05 | ALL threshold-meeting markers (value >= threshold_pct) are collected as anchors in sequence order; the list is immutable for the run. Series with no anchors are flagged no_cross_marker_effect=True. | REQ-CPM-IA-P26.0004 |
| G-06 | Series with no threshold-meeting marker **must** be explicitly flagged (no_cross_marker_effect = True) and included in outputs. | REQ-CPM-IA-P26.0004 |
...
| G-09 | All outputs **must** be in long (tidy) format, uniquely identified by the key (visit_id, point, anchor_marker) plus visit metadata. | REQ-CPM-IA-P26.0010 |
...
| # | Module File | SR Linked | Status | Tests |
|---|-------------|-----------|--------|-------|
| 01 | `src/config_loader.py` | REQ-CPM-IA-P26.0001 | Complete | 21/21 |
| 02 | `src/data_ingestion.py` | REQ-CPM-IA-P26.0002 | Complete | 20/20 |
| 03 | `src/marker_sequence.py` | REQ-CPM-IA-P26.0003 | Complete | 17/17 |
| 04 | `src/trigger_detection.py` | REQ-CPM-IA-P26.0004 | Complete | 19/19 |
| 05 | `src/positive_census.py` | REQ-CPM-IA-P26.0005 | Complete | 13/13 |
| 06 | `src/descent_window.py` | REQ-CPM-IA-P26.0006 | Complete | 23/23 |
| 07 | `src/descent_slope.py` | REQ-CPM-IA-P26.0007 | Complete | 18/18 |
...
- `src/main.py`: aggiunto `_make_per_file_config` e `_run_from_config`; in modalità multi-file Excel il pipeline esegue un run completo per ogni `<file>` con output in `<output_path>/<visit_id>/`; batch resilience: errori per-visita catturati come WARNING, loop continua. 18/18 test (TC-11-006, TC-11-007 aggiunti).
- `src/data_ingestion.py`: `_VALID_POINT_PATTERN` aggiornato — 4° componente opzionale (`(\s*-\s*.+)?`); formato 3 componenti (es. `LH - 3 - c`) ora valido. 20/20 test (TC-02-011 aggiunto).
...
- `src/config_loader.py`: nuovo schema `<input>` con tipo csv/excel/db; `threshold_pct` obbligatorio; `DataQualityConfig`; backward-compat legacy `<input_path>`. 21/21 test.
- `src/data_ingestion.py`: astrazione `DataSource` -> `CsvSource`, `ExcelSource` (melt wide->long da openpyxl), `PostgresSource` stub; `DataQualityReport`; validazione pattern punti anatomici; ritorna `Tuple[DataFrame, DataQualityReport]`. 17/17 test.
- `src/trigger_detection.py`: soglia cambiata da `>` a `>=`; rimosso `break`; nuovo `Anchor` dataclass; `TriggerResult.anchors: List[Anchor]` + `positives_count`; rimossi `first_positive_*`. 19/19 test.
- `src/descent_window.py`: finestra per ancora; nuova regola chiusura su `next_positive`; campo `window_close_reason`; tipo output `List[DescentWindow]`. 23/23 test.
- `src/descent_slope.py`, `src/descent_descriptors.py`, `src/variance_indicator.py`: output `List[...]` per-ancora, formule invariate. 18/18, 15/15, 22/22 test.
- `src/positive_census.py`: aggiunto `total_positives_count`. 13/13 test.
- `src/output_consolidation.py`: granularita detail `(visit, point, ancora)`; nuove colonne `anchor_rank/marker/value/index`, `positives_count`, `window_close_reason`; aggregato con `median_positives_count`; G-09 key `(visit_id, point, anchor_marker)`. 25/25 test.
- `src/main.py`: disaccoppia `(df_raw, dq_report)` da M02; passa `dq_report` a M11. 15/15 test.
- `src/report_generator.py`: parametro `dq_report`; sezione data quality; `anchor_marker` al posto di `first_positive_marker`; descrizione algoritmo aggiornata. 33/33 test.
- `data/config/cpm_ia_config.xml`: tipo `excel` con due file (AF + TP0010); epsilon=1.0; n_max=1000.
- Totale cumulativo: 238/238 test.
...
- Section 4.2 renamed "Most Frequent Trigger Markers by Anatomical Point": filters to `anchor_pct >= 3.5 %` of positive visits per point. Column spec changed to `p{3.5cm}p{7.5cm}rr` with `\footnotesize` and longtable line-breaking. Threshold value (`3.5`) is a template placeholder `__ANCHOR_TABLE_THRESHOLD__`.
...
- Fix: added `_C1_CTRL_DEL = str.maketrans("", "", "".join(chr(i) for i in range(0x80, 0xA0)))` and applied it in `_escape()` before `_LATEX_TRANS`.
- Stripping is correct behaviour; visual garbling (e.g., "ae" sequences in marker names) is a source data quality issue, not a LaTeX issue.
...
- G-09 enforced: uniqueness check on `(visit_id, point, anchor_marker)` — `OutputError` if any key duplicated.
...
- Before segment: `sequence[0:anchor_index]` (excludes anchor), NaN stripped. `variance_before=None` when < 2 clean values remain.
- After segment: `descent_window_values` (includes anchor at position 0), NaN stripped defensively. `variance_after=None` when < 2 clean values.
- Variance formula: `statistics.variance` (Python stdlib, `ddof=1` sample variance by default).
...
- Three mandatory descriptors: `descent_depth`, `descent_length`, `mean_per_step_decrease`.
- `descent_depth = anchor_value - min(window_values)` — uses global minimum, not last value.
- `mean_per_step_decrease = depth / (length-1)` for length > 1; `0.0` for length == 1.
...
- Closed-form OLS formula used directly (from REQ-CPM-IA-P26.0007).
...
- Decision A confirmed by [IR]: NaN values after anchor are bridged.
- Decision B confirmed by [IR]: `truncated_by_n_max=True` only when the n_max cap fires before a natural close.
- Check order in inner loop: (1) NaN skip, (2) rise check, (3) n_max check.
...
- `sorted()` used intentionally for `positive_points` — spec-mandated for deterministic output.
...
- Threshold comparison was strictly `>` (exceeds); updated to `>=` in Rev 00.01.
...
- G-04 enforced via `dict.fromkeys(df[COL_MARKER])` — pure Python ordered-dict pattern.
- `groupby(sort=False)` used to preserve group discovery order without implicit alphabetic sorting.
- 13/13 unit tests pass (`tests/test_marker_sequence.py`).

### Session log — 2026-08-27 Module 02

- `src/data_ingestion.py` implemented: `IngestionError`, column constants, `load_data()`.
- Column names confirmed by [IR]: `visit_id`, `marker`, `point`, `percentage_variation`, `visit_date`.
- `visit_date` kept as string/object — no datetime parsing at ingestion.
...
- `int` cast for `n_max` is strict: `"10.0"` is rejected.
