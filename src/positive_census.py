...
counting and identifying unique points that have at least one first positive
...
class PointCensus:
    ...
    positive_point_count: int        # number of points with at least one first positive
...
def compute_census(trigger_results: TriggerResults) -> PointCensus:
    ...
    Returns:
        PointCensus with positive_point_count, positive_points (sorted), total_point_count.
    """
    ...
    # Points that have at least one visit with a detected first positive.
    ...
    logger.info(
        "CPM-IA census | positive_points=%d/%d | points=%s",
